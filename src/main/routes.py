...
from src.main.forms import UrlCreate, LinkUpdate  # UrlCreated, UrlSubmit
...
def edit_link(id):
    ...
    if form.url_short.data != None and pattern.search(form.url_short.data):
        flash('This URL is invalid ', 'danger')
        return redirect(url_for('main.edit_link', id=link.id))
        # return render_template('main.edit_link.html', title='New link', form=form, legend='New Link')
